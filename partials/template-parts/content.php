...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner'); ?>>
	...
	<header class="entry-header">
		...
		?>
					<small>Published on <?php the_time('j/m/Y'); ?>, in Press releases</small>
				<?php } else {
					$categories = get_the_category();
					$lastElement = array_pop($categories);
				?>
					<small>Αναρτήθηκε στις <?php the_time('j/m/Y'); ?>, στην κατηγορία <a href="<?php echo get_post_type_archive_link(get_post_type()); ?>">Δελτία τύπου</a></small>
				<?php

				}
			endif;
		}
		if (get_post_type() === 'competitions') {
			if (!is_page()) :
				if (ICL_LANGUAGE_CODE == 'en') {
					$categories = get_the_category();
					$lastElement = array_pop($categories);
				?>
					<small>Published on <?php the_time('F j, Y'); ?>, in Competitions</small>
				<?php } else {
					$categories = get_the_category();
					$lastElement = array_pop($categories);
				?>
					<small>Αναρτήθηκε στις <?php the_time('j F, Y'); ?>, στην κατηγορία <a href="<?php echo get_post_type_archive_link(get_post_type()); ?>">Διακηρύξεις - Διαγωνισμοί</a></small>
		<?php
	...
	<div class="row">
		...
		<?php 
		$slides = get_field('post_slider_slider'); 
		$showImg = get_field('display_feat_img');
		?>
		<div class="article-content">
			...
			<?php the_content(); ?>
			<?php
			$contact = get_field('contact_category');
			...
			foreach ($contacts as $contact) {
				array_push($contact_titles, $contact->post_title);
			}
			?>

			<?php if(sizeof($contact_titles) > 0){ ?>
				<div class="contact-list">
					...
					<div class="results">
						...
						<?php foreach($contacts as $contact){ ?>
							<div class="single-item" data-title="<?php echo get_the_title($contact->ID); ?>">
								<div class="left">
								...
								<div class="right">
									<?php if (get_field('email', $contact->ID)) { ?>
										<a href="mailto:<?php echo get_field('email', $contact->ID); ?>" class="entry email"><i class="fas fa-envelope" aria-hidden="true"></i>
											<p><?php echo get_field('email', $contact->ID); ?></p>
										</a>
									<?php } ?>
									<?php if (get_field('phone', $contact->ID)) { ?>
										<a href="tel:<?php echo get_field('phone', $contact->ID); ?>" class="entry tel"><i class="fas fa-phone" aria-hidden="true"></i>
											<p><?php echo get_field('phone', $contact->ID); ?></p>
										</a>
									<?php } ?>
									<?php if (get_field('fax', $contact->ID)) { ?>
										<a href="fax:<?php echo get_field('fax', $contact->ID); ?>" class="entry fax"><i class="fas fa-print" aria-hidden="true"></i>
											<p><?php echo get_field('fax', $contact->ID); ?></p>
										</a>
									<?php } ?>
									<?php if (get_field('address', $contact->ID)) { ?>
										<div class="entry address"><i class="fas fa-map-marked-alt" aria-hidden="true"></i>
					...
					<script>
						var list = <?php echo json_encode($contact_titles); ?>;
						console.log(list);
						// Get input element
					...
					foreach ($attachments as $media) { ?>
						...
						?>
							<a class="attachment file" href="<?php echo $media['file']['url']; ?>" target="_blank" title="<?php echo $media['file']['filename']; ?>">
								...
								<p><?php echo $media['file']['title']; ?> <i class="fas fa-external-link-alt" aria-hidden="true"></i></p>
							</a>
